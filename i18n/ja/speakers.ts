...
export const SESSION_SPEAKERS: Speaker[] = [
  ...
  },
  {
    ...
    color: "navy",
    talkTitle: "最高の DX -\n typed-router と Pinia Colada で実現する次世代 Vue/Nuxt 開発",
...
export const LT_SPEAKERS: Speaker[] = [
  ...
  },
  {
    ...
    talkOverview: "chocoZAPでは、これまでセルフエステやセルフ脱毛の予約に外部サービスを利用していましたが、この度Nuxtで完全内製化しました。\nWebとアプリ（WebView）それぞれに最適化したUIを導入し、毎日利用するからこそ「直感的でスムーズに操作できる」ことを重視しています。\n本LTでは、プロジェクトで実際に取り組んだ内容と、そこで工夫したポイントをお話しします。\n\n以下の話を想定しています。（時間の都合により内容に変更の可能性がございます）\n- Nuxtでの画面構成と状態管理の設計\n  - 例：コンポーネントの分割（views, templatesなど）により、コードの再利用性と保守性を高めることを目指したこと\n  - 例：server/ ディレクトリを活用したAPI通信の整理などの話\n- 内製化によって可能になった柔軟な改善サイクル\n  - 例：APIレスポンスに応じて文言やボタンの挙動を細かく制御できるようになったことによるUI改善\n  - 例：正常系だけでなく異常系のパターン（上限予約数など）にも柔軟に対応でき、ユーザーが混乱しないUIに改善できたこと\n- chocoZAPのデザインシステムとの整合性\n  - 例：自社のデザインシステムに沿ったUI実装が可能になり、他画面との一貫性やブランド体験の統一が図れたこと\n- ユーザーの声を即座に反映できた話\n  - 例：フィードバックを迅速に反映し、不便やストレスを最小限に抑えたこと",
    socialUrls: {},
  },
];
